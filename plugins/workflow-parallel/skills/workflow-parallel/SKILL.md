...
description: "8-stage parallel orchestration master skill. Trigger on: /workflow:parallel, parallel orchestrate, multi-ticket parallel, wave dispatch, parallel implementation. Drives plan-intake → plannotator → codebase-explore → DAG/Wave breakdown → Jira ticket create → git worktree setup → cmux pane dispatch → orchestrate-monitor by invoking each wp-stage<N>-* sub-skill in sequence with confirm gates."
---

# /workflow:parallel — 8-Stage Parallel Orchestration

> 사용자의 rough plan 또는 plan 파일을 받아 8 단계로 정제 / 분할 / 병렬 dispatch / 모니터링하는 마스터 워크플로우. 각 stage 는 sub-skill 로 분리되어 partial restart 가능.
>
...
```
Skill(skill="wp-stage3-codebase-explore")
```
...
```
Skill(skill="wp-stage4-task-breakdown")
```
...
### Stage 5 — ticket-create (병렬)

```
Skill(skill="wp-stage5-ticket-create")
```
...
```
Skill(skill="wp-stage6-worktree-setup")
```
...
```
Skill(skill="wp-stage7-dispatch")
```
...
```
Skill(skill="wp-stage8-orchestrate-monitor")
```
...
```
/wp:stage5-ticket-create   # ticket 생성만 다시
/wp:stage7-dispatch         # dispatch 만 다시
```
...
| 7 | pane 매핑 | inline 매핑 테이블 |
| 8 | PR 링크 / Wave 상태 | inline 보고 |
...
Stage 2 (plannotator-loop)  ← optional 3 round loop
   ↓
Stage 3 (codebase-explore)  ← 병렬 (작업당 1 Explore)
   ↓
Stage 4 (task-breakdown)
   ↓
Stage 5 (ticket-create)     ← 병렬 (Jira API)
   ↓
Stage 6 (worktree-setup)    ← sequential (lockfile / pnpm store)
   ↓
Stage 7 (dispatch)          ← sequential (cmux state race 방지)
   ↓
Stage 8 (orchestrate-monitor)  ← on-demand polling
   ↺ Wave 2 → Stage 6 부터 다시
```

## 핵심 결정 (PLAN.md §5 / IMPL.md 반영)

- plugin slash: `/workflow:parallel` (메인) + `/wp:stage<N>-<name>` (sub)
- Stage 5 ticket 본문 언어: **한국어**
- Stage 6 worktree 위치: `<repo>/../<repo>-<TICKET-KEY>/` (디스크상 형제)
- Stage 7 cmux pane 부족 시: **사용자 수동 split** (자동 split 금지)
- Stage 8 모니터링: **on-demand** (auto polling 금지, 사용자 명시 시만)
